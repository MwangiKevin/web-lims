app.controller('fcdrrCtrl', ['$stateParams', '$state', '$rootScope', '$scope', '$http', 'ngProgress', 'Filters', 'Commons', '$activityIndicator', 'API', 'SweetAlert', 'notify', 'Restangular', 'apiAuth', function($stateParams, $state, $rootScope, $scope, $http, ngProgress, Filters, Commons, $activityIndicator, API, SweetAlert, notify, Restangular, apiAuth) {
    ...
    $scope.showYear = function() {
        var objDate = new Date($scope.fcdrr.from_date);
        locale = "en-us", year = objDate.getFullYear();
        return year;
    }
...
;
    ...
    $scope.promise = baseCommodities.getList({
        fcdrr_format: true,
        reportingOnly: true
    }).then(function(com) {
    ...
    $scope.getSelectableMonths = function() {
        ...
        nowNormalized = moment().startOf("month"), startDateNormalized = moment().subtract('month', $scope.months_back_reporting).startOf('month');
        while (startDateNormalized.isBefore(nowNormalized)) {
            if ($scope.selected.year && (startDateNormalized.format("YYYY") == $scope.selected.year.value)) {
                $scope.selectableDates.months.push({
                    label: startDateNormalized.format("MMMM"),
                    value: startDateNormalized.format("MM")
                });
            }
    ...
    $scope.getSelectableYears = function() {
        nowNormalized = moment().startOf("month"), startDateNormalized = moment().subtract('month', $scope.months_back_reporting).startOf('month');
        $scope.selectableDates.years.push({
        ...
        if (startDateNormalized.format("YYYY") != nowNormalized.format("YYYY")) {
            $scope.selectableDates.years.push({
                label: nowNormalized.format("YYYY"),
                value: nowNormalized.format("YYYY")
            });
        }
