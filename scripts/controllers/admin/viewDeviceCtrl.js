app.controller('viewDeviceCtrl',
    ...
    $scope.device_id = $stateParams.id;

    $scope.populateDevice_details = function() { // fetch device details based on the id
        if ($stateParams.id > 0) {
            ...
            loaded_facility_device.get().then(function(facility_device_load) {
               ...
               if($scope.facility_dev_detail.facility_rollout_status == 1 || $scope.facility_dev_detail.status == 1){
                    ...
                    $scope.facility_dev_detail.facility_rollout_status = 'Yes';
               }
               else{
                    $scope.facility_dev_detail.facility_rollout_status = 'No';
               }
            })
        }
    }
